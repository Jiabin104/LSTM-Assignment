...
"""
Minimal character-level LSTM model. Written by Jiabin Xu
...
# data I/O
data = open('data_input.txt', 'r').read()  # should be simple plain text file
...
def backward(activations, clipping=True):
    ...
    for t in reversed(range(input_length)):
        # initialize dzs
        dzs = np.zeros_like(zs)

        # computing the gradients here
        dys = ps[t] - ls[t]
        #np.add(dWhy, np.dot(dys, hs[t].T), out=dWhy, casting='unsafe')
        dWhy += np.dot(dys,hs[t].T)
        #np.add(dby, np.sum(dys, axis=1, keepdims=True), out=dby, casting='unsafe')
        ...
        dos_pre_activation = dos * dsigmoid(os[t])
        dWo+=np.dot(dos_pre_activation, zs[t].T)
        #np.add(dWo,np.dot(dos_pre_activation,zs[t].T),out=dWo,casting='unsafe')
        dbo+=np.sum(dos_pre_activation, axis=1, keepdims=True)
        #np.add(dbo, np.sum(dos_pre_activation, axis=1, keepdims=True), out=dbo, casting='unsafe')
        dzs = np.dot(Wo.T, dos_pre_activation) + dzs

        # memory
        dcs = dhs * os[t] * dtanh(np.tanh(cs[t])) + dcnext

        # foget gate
        dfs = dcs * cs[t-1]
        dfs_pre_activation = dfs * dsigmoid(fs[t])
        dWf+=np.dot(dfs_pre_activation, zs[t].T)
        #np.add(dWf, np.dot(dfs_pre_activation, zs[t].T), out=dWf, casting='unsafe')
        dbf+=np.sum(dfs_pre_activation, axis=1, keepdims=True)
        #np.add(dbf, np.sum(dfs_pre_activation, axis=1, keepdims=True), out=dbf, casting='unsafe')
        dzs = np.dot(Wf.T, dfs_pre_activation) + dzs

        # input gate
        dins = dcs * c_s[t]
        dins_pre_activation = dins * dsigmoid(ins[t])
        dWi+= np.dot(dins_pre_activation, zs[t].T)
        #np.add(dWi, np.dot(dins_pre_activation, zs[t].T), out=dWi, casting='unsafe')
        dbi+=np.sum(dins_pre_activation, axis=1, keepdims=True)
        #np.add(dbi, np.sum(dins_pre_activation, axis=1, keepdims=True), out=dbi, casting='unsafe')
        dzs = np.dot(Wi.T, dins_pre_activation) + dzs

        dc_s = dcs * ins[t]
        dc_s_pre_activation = dc_s * dtanh(c_s[t])
        dWc+=np.dot(dc_s_pre_activation, zs[t].T)
        #np.add(dWc, np.dot(dc_s_pre_activation, zs[t].T), out=dWc, casting='unsafe')
        dbc+=np.sum(dc_s_pre_activation, axis=1, keepdims=True)
        #np.add(dbc, np.sum(dc_s_pre_activation, axis=1, keepdims=True), out=dbc, casting='unsafe')
        dzs = np.dot(Wc.T, dc_s_pre_activation) + dzs

        # back to last dcs
        dcsnext = dcs * fs[t]

        # back to last dh and embeding
        dhsnext = dzs[: hidden_size, :]
        dwes = dzs[hidden_size :, :]
        np.add(dWex, np.dot(dwes, xs[t].T), out=dWex, casting='unsafe')
